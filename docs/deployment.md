...
> **Unverified assumption: claude.ai actually delivers the password through `state`.**
> `MCP_OAUTH_PASSWORD` is checked against the OAuth `state` query parameter of the
> `/authorize` request (see [README > Authentication](../README.md#authentication)).
> `state` is generated by the OAuth *client* - Claude's own backend/browser flow during
> **Settings → Connectors → Add custom connector** - not typed in by you, and this repo
> has no way to drive a real claude.ai session in CI or in tests, so whether Claude's
> official connector setup lets you influence `state` at all (and therefore whether this
> password gate is reachable in practice, as opposed to merely present in the code) has
> only been reasoned about, never confirmed against production claude.ai (D2).
>
> **How to verify it yourself:** with `MCP_OAUTH_PASSWORD` set and the server reachable
> (steps 1-4 above), add the custom connector in claude.ai and, right when the OAuth
> prompt would appear, capture the actual `/authorize` request - either your browser's
> network tab (DevTools → Network → filter `authorize`) or a one-off `tcpdump`/reverse
> proxy log on the LXC (the server itself deliberately does not log query strings, see
> the access-log note above) - and confirm a `state=` parameter is present and contains
> the string you set as `MCP_OAUTH_PASSWORD`. If authorization then succeeds, the
> assumption holds for your Claude client version at the time of testing.
>
> **If `state` turns out not to carry it:** the connector will fail to authorize (or
> silently ignore your password) with no clear error surfaced to you beyond "error
> connecting" - do **not** work around this by setting `MCP_OAUTH_ALLOWED_REDIRECT_DOMAINS`
> more permissively or otherwise weakening the redirect-domain check; that check is not a
> security boundary on its own (see README) and loosening it does not compensate for a
> non-functional password gate. Instead, treat this as a blocker: keep the server
> `localhost`-only (no Funnel) until a replacement is in place, and open a follow-up work
> package to design a real consent-page/login step for `PersonalAuthProvider`'s
> `/authorize` handler (D2 in [docs/improvement-plan.md](improvement-plan.md)) instead of
> relying on a client-supplied OAuth parameter for the only access gate.
